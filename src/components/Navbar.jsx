              ...
              <div className="relative user-dropdown">
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 mt-3 w-44 bg-white border border-gray-200 rounded-lg shadow-md text-sm text-black z-50">
                    ...
                    <Link to="/orders" onClick={() => setUserMenuOpen(false)} className="block px-4 py-2 hover:bg-gray-100">{t("orders")}</Link>
                    ...
                    {userRole === "store" && (
                      <Link to="/store/products" onClick={() => setUserMenuOpen(false)} className="block px-4 py-2 hover:bg-gray-100">{t("storeProducts")}</Link>
                    )}
                    <button onClick={() => { localStorage.removeItem("token"); window.location.href = "/login"; }} className="block w-full text-left px-4 py-2 hover:bg-gray-100">{t("logout")}</button>
                  </div>
                )}
              </div>

              <span className="text-sm text-gray-600 font-medium ml-2">
                {t("balance")}: <span className="text-black">${balance.toFixed(2)}</span>
              </span>
            </>
          ) : (
            <>
              <Link to="/login" className="px-4 py-2 border border-black rounded hover:bg-black hover:text-white transition">{t("signIn")}</Link>
              <Link to="/register" className="px-4 py-2 bg-black text-white rounded hover:bg-gray-800 transition">{t("signUp")}</Link>
            </>
          )}
        </div>
        ...
        {open && (
          ...
          >
            ...
            <ul className="px-6 py-6 space-y-4 font-medium">
              ...
              {!token && (
                <>
                  <li><Link to="/login" onClick={() => setOpen(false)} className="block py-2 hover:text-gray-400">{t("signIn")}</Link></li>
                  <li><Link to="/register" onClick={() => setOpen(false)} className="block py-2 hover:text-gray-400">{t("signUp")}</Link></li>
                </>
              )}
            </ul>
          </motion.div>
